...
void avlTree::insert(avl_node* head, int v)
{
	if (root == NULL)
	{
		root = new avl_node();
...
int main()
{
	...
	string HashSArr[10];
	int Counter = 0;
	sf::RenderWindow window(VideoMode(1000, 500), "Visualizer");
	...
	for (int i = 0; i < 7; i++)
	{
		...
		DisOutput[i].setFillColor(Color::Green);
		DisOutput[i].setCharacterSize(20);
	}
	for (int i = 0; i < 10; i++)
	{
		HashDis[i].setFont(font);
		HashDis[i].setFillColor(Color::Blue);
		HashDis[i].setCharacterSize(20);
	}
	Picked.setFont(font);
	Picked.setFillColor(Color::Yellow);
	...
	while (window.isOpen())
	{
		...
		window.draw(text[2]);
		Del.setString("To stop delete press W");
		...
		if (Keyboard::isKeyPressed(Keyboard::Enter))
		{
			...
			if (BCondi || ACondi)
			{
				for (int i = 0; i < 7; i++)
				{
					if (Display[i] != 0)
					{
						DataSArr[i] = to_string(Display[i]);
					}
					else
					{
						DataSArr[i] = "0";
			...
			if (HCondi && (Lin || Quad))
			{
				...
				for (int i = 0; i < 10; i++)
				{
					if (Display[i] != -1)
					{
						HashSArr[i] = to_string(Display[i]);
					}
					else
					{
						HashSArr[i] = "0";
		...
		{
			IncDec.setString("Left to Dec Press A | Right to Inc Press D");
			stringstream ss;
			ss << Data;
			ss >> DataS;
